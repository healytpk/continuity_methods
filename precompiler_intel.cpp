...
protected:
    ...
    string_view Full_Param_List(void) const
    {
        ...
        return string_view(&*(s.cbegin() + index), &*(s.cbegin() + i) - &*(s.cbegin() + index));
    }
    ...
    void ProcessParams(void)
    {
        ...
        for ( ; iter != svregex_top_level_token_iterator(); ++iter )
        {
            ...
            _params.emplace_back( string_view(iter->first, iter->second - iter->first) );
...
public:
    ...
    explicit Function_Signature(string_view const arg) : _original(arg)
    {
        ...
        for ( ; iter != svregex_top_level_iterator(); ++iter )
        {
            ...
            string_view const found { (*iter)[1u].first, (*iter)[1u].second - (*iter)[1u].first };
            ...
            for ( size_t location = 0u; location < _original.size(); ++location)
            {
                ...
                _name = string_view( &*(_original.cbegin() + location), &*(_original.cbegin() + location + found.size()) - &*(_original.cbegin() + location));
    ...
    {
        os << string_view( &*_original.cbegin(), &*_name.cend() - &*_original.cbegin());

        os << "____WITHOUT_CONTINUITY";

        os << string_view( &*_name.cend(), &*_original.cend() - &*_name.cend());
    }
    ...
    void Signature_Of_Replacement_Function(T &os) const
    {
        ...
        os << string_view( &*Full_Param_List().cend(), &*_original.cend() - &*Full_Param_List().cend() );
    }
...
#if 1
    if ( "template" == retval.substr(0u, 8u) ) return {};
#else
...
string_view Indentation_For_CurlyPair(CurlyBracketManager::CurlyPair const &cp)
{
    ...
    */
    ...
    return string_view( &g_intact[i_first_char], &g_intact[i] - &g_intact[i_first_char]);  // REVISIT FIX - possible dereference of invalid iterator
}
...
tuple< string, string, list< array<string,3u> >  > Intro_For_Curly_Pair(CurlyBracketManager::CurlyPair const &cp)
{
    ...
    if ( "namespace" == intro.substr(0u, 9u) )
    {
        string str;
    ...
    if (   !( "class" == intro.substr(0u, 5u) || "struct" == intro.substr(0u, 6u) ) ) return {};
...
bool Strip_Last_Scope(string &str)
{
    ...
    */
    ...
    if ( (str.size() < (2*seplen + 1u)) || (separator != str.substr(0u, sizeof(separator) -1)) || (separator != str.substr( str.size() - (sizeof(separator) - 1)) ) ) // minimum = "::A::"
    {
...
string Find_Class_Relative_To_Scope(string &prefix, string classname)
{
    ...
    if ( "::" == classname.substr(0u,2u) )  // If it's an absolute class name rather than relative
    {
...
void Find_All_Usings_In_Open_Space(size_t const first, size_t const last, string scope_name)
{
    ...
    assert( "::" == scope_name.substr(scope_name.size() - 2u) );
...
void Instantiate_Scope_By_Scope_Where_Necessary(string_view str)
{
    ...
    if ( separator == str.substr( str.size() - (sizeof(separator) - 1u) ) ) str.remove_suffix(seplen);  // Turn "::A::B::" into "::A::B"

    if ( (str.size() < (seplen + 1u)) || (separator != str.substr(0u,sizeof(separator) - 1u)) || separator != str.substr( str.size() - (sizeof(separator) - 1u)) )  // minimum = "::A"
    {
        throw runtime_error("Remove_Last_Scope: Invalid string");
    }
          ...
          ++iter )
    {
        ...
        clog << "Level " << i << ": " << string_view(str.cbegin(), iter->second - str.cbegin());

        if ( -1 != string_view(iter->first,iter->second - iter->first).find('<') )  // check if right-most is a template class
        {
            clog << "    <---- template class";
        }
...
void Print_Final_Output(void)
{
    ...
    for ( auto const &ee : g_func_alterations_all )
    {
        for ( auto const &e : ee.second )
        {
            ...
            cout << string_view( &*(g_intact.cbegin() + i), &*mi.iter_first_char - &*(g_intact.cbegin() + i) );

            fs.Original_Function_Signature_Renamed(cout);

            cout << string_view( &*(g_intact.cbegin() + mi.p_body->First()), &*(g_intact.cbegin() + mi.p_body->Last() + 1u) - &*(g_intact.cbegin() + mi.p_body->First()) )
                 << endl
                 << endl;
    ...
    cout << string_view( &*(g_intact.cbegin() + i), &*(g_intact.cend()) - &*(g_intact.cbegin() + i) );
}
